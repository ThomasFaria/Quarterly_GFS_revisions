...
ListofModels8 <-  unlist(lapply(1:length(Regressor), 
                                function(n) combn(Regressor, n, FUN=function(row) 
...
TopMod8 <- list("AIC" = dplyr::bind_rows(lapply(1:length(Items), function(itemNb) {Models8[[itemNb]][order(Models8[[itemNb]]$AIC)[1],]}))|>
                  dplyr::select("Item", "Group", "ModelSpe", "RMSE", "AIC","BIC", "p.value", "N"),
                "BIC" = dplyr::bind_rows(lapply(1:length(Items), function(itemNb) {Models8[[itemNb]][order(Models8[[itemNb]]$BIC)[1],]}))|>
                  dplyr::select("Item", "Group", "ModelSpe", "RMSE", "AIC", "BIC", "p.value", "N")
)
...
Models7 <-list("AIC" = list(),
               "BIC" = list())

for (aCrit in c("AIC", "BIC")) {
  tmp <- TopMod8[[aCrit]]%>%
    mutate(ModelSpe = str_replace_all(ModelSpe, c("\\+Rev_YEN" = "", "\\+Rev_PCN" = "","\\+Rev_ITN" = "", "\\+Rev_EXN" = "","\\+Rev_GCN" = "", "\\+Rev_WGS" = "")),
...
TopMod7 <- list("AIC" = bind_rows(lapply(Items, function(item) {Models7[["AIC"]][[item]]}))%>%
                  select("Item", "Group", "ModelSpe", "RMSE", "AIC","BIC", "p.value", "N"),
                "BIC" = bind_rows(lapply(Items, function(item) {Models7[["BIC"]][[item]]}))%>%
                  select("Item", "Group", "ModelSpe", "RMSE", "AIC", "BIC", "p.value", "N")
)

##### MODEL 6 : First Announcement #####
Models6 <-list("AIC" = list(),
               "BIC" = list())

for (aCrit in c("AIC", "BIC")) {
  tmp <- TopMod7[[aCrit]]%>%
    mutate(ModelSpe = str_replace_all(ModelSpe, c("\\+RevLag1" = "", "\\+RevLag2" = "","\\+RevLag3" = "", "\\+RevLag4" = "","\\+RevLag5" = "")),
...
TopMod6 <- list("AIC" = bind_rows(lapply(Items, function(item) {Models6[["AIC"]][[item]]}))%>%
                  select("Item", "Group", "ModelSpe", "RMSE", "AIC","BIC", "p.value", "N"),
                "BIC" = bind_rows(lapply(Items, function(item) {Models6[["BIC"]][[item]]}))%>%
                  select("Item", "Group", "ModelSpe", "RMSE", "AIC", "BIC", "p.value", "N")
)

##### MODEL 5 : ESA2010 #####
Models5 <-list("AIC" = list(),
               "BIC" = list())

for (aCrit in c("AIC", "BIC")) {
  tmp <- TopMod6[[aCrit]]%>%
    mutate(ModelSpe = str_replace_all(ModelSpe, c("\\+FirstAnnounGr" = "")),
...
TopMod5 <- list("AIC" = bind_rows(lapply(Items, function(item) {Models5[["AIC"]][[item]]}))%>%
                  select("Item", "Group", "ModelSpe", "RMSE", "AIC","BIC", "p.value", "N"),
                "BIC" = bind_rows(lapply(Items, function(item) {Models5[["BIC"]][[item]]}))%>%
                  select("Item", "Group", "ModelSpe", "RMSE", "AIC", "BIC", "p.value", "N")
)

##### MODEL 4 : Quarter FIXED EFFECT #####
Models4 <-list("AIC" = list(),
               "BIC" = list())

for (aCrit in c("AIC", "BIC")) {
  tmp <- TopMod5[[aCrit]]%>%
    mutate(ModelSpe = str_replace_all(ModelSpe, c("\\+ESA2010" = "")),
...
TopMod4 <- list("AIC" = bind_rows(lapply(Items, function(item) {Models4[["AIC"]][[item]]}))%>%
                  select("Item", "Group", "ModelSpe", "RMSE", "AIC","BIC", "p.value", "N"),
                "BIC" = bind_rows(lapply(Items, function(item) {Models4[["BIC"]][[item]]}))%>%
                  select("Item", "Group", "ModelSpe", "RMSE", "AIC", "BIC", "p.value", "N")
)

##### MODEL 3 : COUNTRY FIXED EFFECT #####
Models3 <-list("AIC" = list(),
               "BIC" = list())

for (aCrit in c("AIC", "BIC")) {
  tmp <- TopMod4[[aCrit]]%>%
    mutate(ModelSpe = str_replace_all(ModelSpe, c("\\+ObsQ_1" = "", "\\+ObsQ_2" = "","\\+ObsQ_3" = "","\\+ObsQ_4" = "")),
...
TopMod3 <- list("AIC" = bind_rows(lapply(Items, function(item) {Models3[["AIC"]][[item]]}))%>%
                  select("Item", "Group", "ModelSpe", "RMSE", "AIC","BIC", "p.value", "N"),
                "BIC" = bind_rows(lapply(Items, function(item) {Models3[["BIC"]][[item]]}))%>%
                  select("Item", "Group", "ModelSpe", "RMSE", "AIC", "BIC", "p.value", "N")
)

##### MODEL 2 : NAIVE #####

Models2 <-list("AIC" = list(),
               "BIC" = list())

for (aCrit in c("AIC", "BIC")) {
  tmp <- TopMod3[[aCrit]]%>%
    mutate(ModelSpe = str_replace_all(ModelSpe, c("\\+Country_DE" = "", "\\+Country_ES" = "","\\+Country_FR" = "", "\\+Country_IT" = "","\\+Country_NL" = "", "\\+Country_BE" = "", "\\+Country_AT" = "","\\+Country_FI" = "", "\\+Country_PT" = "")),
...
TopMod2 <- list("AIC" = bind_rows(lapply(Items, function(item) {Models2[["AIC"]][[item]]}))%>%
                  select("Item", "Group", "ModelSpe", "RMSE", "AIC","BIC", "p.value", "N"),
                "BIC" = bind_rows(lapply(Items, function(item) {Models2[["BIC"]][[item]]}))%>%
                  select("Item", "Group", "ModelSpe", "RMSE", "AIC", "BIC", "p.value", "N")
)
